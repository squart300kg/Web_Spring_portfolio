...
<div class = "container">
	<div class="box box-primary" style="padding:10px;" >
			<div class="box-header">
				<h3 class="box-title">관절의 이해 게시판</h3>
		...
		<script>
			...
			$(document).ready(function(){
				...
				$(".btn-warning").on("click",function(){
					formObj.attr("action","<%=request.getContextPath()%>/board2/update");
					formObj.attr("method","get");
					formObj.submit();
				});
				$(".btn-danger").on("click",function(){
					formObj.attr("action","<%=request.getContextPath()%>/board2/delete");
					formObj.submit();
				});
				$("#bo").on("click",function(){
					self.location = "<%=request.getContextPath()%>/board2/list?page=${criteria.page}";
				});
	...
	<script>
		   $(document).ready(function() {
		    ...
		    $.ajax({
					url : "<%=request.getContextPath()%>/reply2" ,
					type : "POST",
					dataType: "json",
				...
				<table class="table table-bordered table-hover" id="replyTable">
					...
					<!-- 	<input type="button" value="댓글 접기/펼치기" onclick="fn_roll(${replyVO.rnum});"> -->
						...
						<c:if test=  "${replyVO.writer == User.id}">
							...
							&nbsp;&nbsp;
							<input type="button" value="답글수정"  name="modify"  id="modif" onclick="document.location.href='<%=request.getContextPath()%>/board2/updateReR?num=${num}&rnum=${replyVO.rnum}&page=${criteria.page}'" >
							&nbsp;&nbsp;
						</c:if>	
						<c:if test=  "${! empty User}">
							<input type="button" value="좋아요" onclick="document.location.href='<%=request.getContextPath()%>/board2/like?num=${num}&rnum=${replyVO.rnum}&page=${criteria.page}'"><br/>
						</c:if>	
							</td>
				 ...
				 function fn_delete(rnum){
					 ...
					 if(confirm("삭제하겠습니까?")){
						  <c:forEach var="replyVO" items="${rlist}" varStatus="status">
				    	   location.href = '<%=request.getContextPath()%>/board2/delRe?num=${num}&rnum='+rnum+'&page=${criteria.page}';
				  		  </c:forEach>
					 }
					};
