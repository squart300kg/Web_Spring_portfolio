	...
	<script>
			...
			$(document).ready(function(){
				...
				$("#bo").on("click",function(){
					self.location = "<%=request.getContextPath()%>/board4/insert";
				});
			...
			<div class="box">
				...
				<div class="box-body">
					...
					<c:forEach items="${slist}" var="boardVO">
						<tr> 
							<td>${boardVO.num}</td>
							<td><a href="<%=request.getContextPath() %>/board4/read?num=${boardVO.num}&page=${searchCriteria.page}">${boardVO.subject}</a></td>
	...
	<div class="text-center">
		<ul class="pagination">
			<c:if test="${pageMaker.prev}">
				<li> <a href="<%=request.getContextPath()%>/board4/list${pageMaker.makeSearch(pageMaker.startPage -1)}">이전</a></li>
			</c:if>
			<c:forEach begin="${pageMaker.startPage}" end="${pageMaker.endPage}" var="idx">
				<li <c:out value="${pageMaker.criteria.page == idx ? 'class=active' : '' }"/>>
					<a href="<%=request.getContextPath()%>/board4/list${pageMaker.makeSearch(idx)}">${idx}</a>
				</li>	
			</c:forEach>
			<c:if test="${pageMaker.next && pageMaker.endPage > 0}">
				<li> <a href="<%=request.getContextPath()%>/board4/list${pageMaker.makeSearch(pageMaker.endPage + 1)}">다음</a></li>
			</c:if>
...
<div class="box-footer">
	...
	$(document).ready(function(){
		$("#searchBtn").on("click",function(event){
			self.location = "<%=request.getContextPath()%>/board4/list${pageMaker.makeQuery(1)}"
